...
class Biped3D(object):
    def __init__(self):
        ...
        p.resetDebugVisualizerCamera(cameraDistance=0.5, cameraYaw=120,
                                     cameraPitch=-30, cameraTargetPosition=[0, 0, 0.2])
        p.setAdditionalSearchPath(pybullet_data.getDataPath())  # optionally

        self.ground = p.loadURDF("plane.urdf")
        self.robot = p.loadURDF("/yanshee/robots/yanshee3_2.urdf", [0, 0, 0], [0, 0, 0, 1], useFixedBase=False)
    ...
    def joint_controller(self, t):
        a = np.deg2rad(30)
        q_d_haa = 0.5 * a*np.sin(2*np.pi*self.motion_f*t) + 0.75*a
        ...
        dq_d_haa = np.pi*self.motion_f*a*np.cos(2*np.pi*self.motion_f*t)
