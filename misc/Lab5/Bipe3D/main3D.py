...
class Biped3D(object):
    def __init__(self):
        ...
        p.resetDebugVisualizerCamera(cameraDistance=1.2, cameraYaw=0,
                                     cameraPitch=0, cameraTargetPosition=[0, 0, 0.5])
    ...
    def step(self, torque_array):
        ...
        p.resetDebugVisualizerCamera(cameraDistance=1.2, cameraYaw=0,
                                     cameraPitch=0, cameraTargetPosition=[self.robo_pose-0.5, 0, 0.5])
        p.stepSimulation()
    ...
    def joint_controller(self, t):
        ...
        q_d_vec = np.array([q_d_haa, q_d_hfe, q_d_kfe, q_d_haa, q_d_hfe, q_d_kfe])
        ...
        dq_d_vec = np.array([dq_d_haa, dq_d_hfe, dq_d_kfe, dq_d_haa, dq_d_hfe, dq_d_kfe])

        # PD controller gains
        k_vec = np.array([100, 50, 50, 100, 50, 50])
        b_vec = np.array([0.05, 0.05, 0.05, 0.05, 0.05, 0.05])
