...
class Biped2D(object):
    ...
    def joint_controller(self, t):
        ...
        q_d_vec = np.array([theta_1, -theta_2, np.pi/2-(np.pi/2-theta_2+theta_1), theta_3, -theta_4, np.pi/2-(np.pi/2-theta_4+theta_3)])
        J = np.array([[-l1*np.sin(theta_1)+l2*np.sin(theta_2-theta_1), -l2*np.sin(theta_2-theta_1)],
                      [l1*np.cos(theta_1)+l2*np.cos(theta_2-theta_1), -l2*np.cos(theta_2-theta_1)]])
        dx = -b*np.sin(theta)
        dy = a*np.sin(theta)
        dtheta1, dtheta2 = np.linalg.inv(J) @ np.array([dx, dy]).T
        J2 = np.array([[-l1*np.sin(theta_3)+l2*np.sin(theta_4-theta_3), -l2*np.sin(theta_4-theta_3)],
                      [l1*np.cos(theta_3)+l2*np.cos(theta_4-theta_3), -l2*np.cos(theta_4-theta_3)]])
        dx2 = -b*np.sin(theta2)
        dy2 = a*np.sin(theta2)
        dtheta3, dtheta4 = np.linalg.inv(J2) @ np.array([dx2, dy2]).T
        dq_d_vec = np.array([dtheta1, dtheta2, -(-dtheta2+dtheta1), dtheta3, dtheta4, -(-dtheta4+dtheta3)])
